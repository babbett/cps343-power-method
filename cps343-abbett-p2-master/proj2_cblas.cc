/* $Smake: g++ -Wall -O2 -o %F %f -lhdf5 -lcblas -latlas -fopenmp
 ...
 * 
 * This is a parallelized version of the original (project 1) cblas program.
...
void mat_vect_mul(double* y, double* A, double* x, int n, int m) {
	...
	1.0,	/* alpha */
	A,	    /* the matrix A */
	n,	    /* the leading dimension of A */
	x,	    /* the vector x */
	1,	    /* increment of x */
	0.0,	/* beta */
	y,	    /* the return vector y */
	1);	    /* increment of y */
}

double dot_prod(double* x, double* y, int n) {
    // Compute new estimate
    double lambda_new = cblas_ddot(
        n, /* The number of elements in the vectors */
        x, /* The vectors x */
        1, /* The stride within x */
        y, /* The vectors x */
        1  /* The stride within y */
    );
    return lambda_new;
}

double two_norm(double* x, int n) {
    double norm = cblas_dnrm2(
...
void scale(double* x, int n, double factor) {
    cblas_dscal(
        n,      /* The number of elements in the vector */
        1/factor, /* The scaling factor alpha */
        x,      /* The vector */
        1      /* The stride */
    );
}
